...
function generateDBTypeFile() {
  try {
    console.log("🔍 Scanning schema files for type generation...");
    ...
    if (schemaFiles.length === 0) {
      console.log("⚠️  No schema files found for type generation");
      return;
    }
    ...
    for (const file of schemaFiles) {
      ...
      if (exportMatch) {
        ...
        console.log(`   ⚠️  Could not parse export from ${file}`);
      }
    }
    console.log();
...
function main() {
  ...
  console.log(`🔷 Types target: ${path.join(projectRoot, "db.types.ts")}`);
